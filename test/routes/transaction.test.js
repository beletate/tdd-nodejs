...
describe('Ao tentar inserir uma transação inválida', () => {
    ...
    test('Não deve inserir uma transação com tipo inválido', () => testTemplate({type: 'X'}, 'O tipo é inválido.'));
})
...
test('Não deve remover conta com transação', () => {
    return app.db('transactions').insert(
        { description: 'T NOT DELETED', date: new Date(), ammount: 100, type: 'I', acc_id: accUser.id }, ['id']
